...
app.post('/addItem', urlencodedParser, (req, res) => {
  ...
  readData('./users.json', data => {
    ...
    all.forEach(item => {
      ...
      let cx = item.data.split("-");
      ddlData[cx[0]].data[cx[1]].push(item);
    })
    // console.log(ddlData)
    writeData('./ddle.json',JSON.stringify(ddlData), _=>{
      console.log("更新ddle.json成功")
    })
...
http.listen('3013', () => console.log('启动在 3013 端口'))
